...
class Mapping:
    ...
    def find_smallest_mean_squared_error(self):
        ...
        if smallest_mean_squared_error == None:
            raise ValueError(
                "error: smallest_mean_squared_error could not be calculated"
            )

    def find_matching_test_coordinates(self):
        if self.max_delta is None or self.ideal_index is None:
            raise ValueError(
                "error: max_delta or ideal index is not set, cannot find matching test coordinates"
            )
    ...
    def find_max_delta(self):
        if self.ideal_index == None:
            raise ValueError(
                "error: self.index is not set, cannot calculate find_max_delta"
            )
