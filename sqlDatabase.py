...
class SqlDatabase:
    ...
    def write(self):
        ...
        self.ideal_dataframe.to_sql(ideal_table_name , con=self.engine, index=False, if_exists='replace')
        ideal_table = sqlalchemy.Table(ideal_table_name , self.meta, autoload_with = self.engine)
        

        test_table_name = "test" 
        self.test_dataframe.to_sql(test_table_name , con=self.engine, index=False, if_exists='replace')
        test_table = sqlalchemy.Table(test_table_name , self.meta, autoload_with = self.engine)
